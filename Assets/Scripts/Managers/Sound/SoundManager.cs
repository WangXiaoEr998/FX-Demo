...
public class SoundManager : MonoBehaviour {
	...
	private static List<Sound> _AllSounds;
 ...
 // 	}
	...
	void Awake(){
		...
		_AllSounds.AddRange(AllSoundsHolder);
		_AllSounds.AddRange(AllStreamSoundsHolder);
		_AllSounds.AddRange(AllResourceSoundsHolder);
	...
	private void Start() {
		Observable.DelayLoop(10, CheckUnload);
	}
	...
	void LateUpdate(){
		...
		for(int i = 0; i < _AllPlaying.Count;i++) {
			var sound = _AllPlaying[i];
			if(!sound.pauseEffect){
				if(sound.currentSource == null && !sound.loop){
					_AllPlaying.RemoveAt(i); i--;
					continue;
				}
				if(sound.currentSource != null){
					// if(sound.fadeIn){
					// 	sound.controllerFadeIn++;
					// 	sound.currentSource.volume = sound.controllerFadeIn/(sound.timeToFadeIn*SoundManager.fps);
					// 	if(sound.currentSource.volume >= 1) sound.fadeIn = false;
					// }
					
					if(sound.stopAt > 0){
						if (sound.currentSource.time >= sound.stopAt) {
							sound.currentSource.Stop();
						}
					}
					
					// if(sound.fadeOut){
					// 	if(sound.currentSource.time >= sound.controllerFadeOut){
					// 		sound.fadeOutTimer++;
					// 		sound.currentSource.volume = 1 - sound.fadeOutTimer/(sound.timeToFadeOut*SoundManager.fps);
					// 		if(sound.currentSource.volume <= 0) sound.fadeOut = false;
					// 	}
					// }
					
					// if(System.Math.Round(sound.currentSource.time,2) >= System.Math.Round(sound.timeToTrigger,2) && sound.canShootEvent){
					// 	GameObject toRecive = (sound.targetForTrigger != null) ? sound.targetForTrigger : gameObject;
					// 	string funcToTrigger = (sound.functionForTrigger != null) ? sound.functionForTrigger : "OnSoundTrigger";
					// 	toRecive.SendMessage(funcToTrigger, sound.name, SendMessageOptions.DontRequireReceiver);
					// 	sound.canShootEvent = false;
					// }
					//
					if(!sound.currentSource.isPlaying){
						sound.currentSource.clip = null;
	...
	static public void RemoveSoundByName(string n){
		...
		// }
		// _AllPlaying = sHolder;
		_AllSounds.RemoveAll(sound => sound.name == n);
		_AllPlaying.RemoveAll(sound => sound.name == n);
	}
	...
	static public void RemoveAllNormalSounds(){
		...
		bool Predicate(Sound sound) => !(sound.isStreamSound && sound.isResourceSound);
		RemoveAllSounds(_AllSounds, Predicate);
		RemoveAllSounds(_AllPlaying, Predicate);
	}

	static public void RemoveAllSounds(List<Sound> sounds, Predicate<Sound> predicate) {
		for(int i = 0; i < sounds.Count;i++){
			if(predicate(sounds[i])){
				ScriptableObject.Destroy(sounds[i]);
				sounds.RemoveAt(i);
				i--;
			}
		}
	...
	static public void RemoveAllStreamSounds(){
		...
		bool Predicate(Sound sound) => sound.isStreamSound;
		RemoveAllSounds(_AllSounds, Predicate);
		RemoveAllSounds(_AllPlaying, Predicate);
	}
	static public void RemoveAllResourceSounds(){
		...
		// }
		// _AllPlaying = sHolder;
		bool Predicate(Sound sound) => sound.isResourceSound;
		RemoveAllSounds(_AllSounds, Predicate);
		RemoveAllSounds(_AllPlaying, Predicate);
	}
	static public void RemoveAllSounds(){
		...
		// _AllPlaying.Clear();
		bool Predicate(Sound sound) => true;
		RemoveAllSounds(_AllSounds, Predicate);
		RemoveAllSounds(_AllPlaying, Predicate);
		_AllSounds.Clear();
		_AllPlaying.Clear();
	...
	static private Sound[] GetSounds(string clipName)
	{
		...
		// 	toReturn[i] = hold[i];
		// }
		Sound[] toReturn = _AllSounds.FindAll(sound => string.Equals(sound.name, clipName)).ToArray();
	...
	static public void Stop(soundTrack track)
	{
		_AllPlaying.FindAll(x => x.track == track).ForEach(x => x.currentSource.Stop());
	}
	
	/// <summary>
	/// Play the (name) sound.
	/// </summary>
	static Sound Play(string name)
    {
	...
	/// </summary>
	static Sound Play(string name, float delay)
    {
	...
	/// </summary>
	static Sound Play(string name, float delay, float playAt)
    {
	...
	/// </summary>
	static Sound Play(string name, float delay, float playAt, float stopAt)
    {
  ...
  //               soundsFound.Add(_AllSounds[i]);
		// }
		var soundsFound = _AllSounds.FindAll(sound => string.Equals(sound.name, name));
		...
		//TODO:等待AB资源管理 完善后 去修改Resources.Load
		AudioClip toPlay = (!playingSound.isResourceSound) ? playingSound.clip : Resources.Load<AudioClip>(playingSound.nameInResourceFolder) as AudioClip;
  ...
  //
		...
		if (_currentPlaying == null) {
			...
			// if(!playingSound.loop)
			// 	GameObject.Destroy(holderSound, toPlay.length+1.0f);

			_currentPlaying = holderSound.AddComponent<AudioSource>(); 
			_pool.Add(_currentPlaying);
		}
	...
	public static void Play(string localPath, soundTrack track, bool loop = false, float delay = 0) {
		var clipName = localPath;
		var sound = GetSound(clipName);
		if (sound == null) {
			var clip = Resources.Load<AudioClip>(localPath);
			if (clip == null) {
				return;
			}
			sound = AddSound(clip, clipName, track, language, loop,
				false, 0, false, 0, false, Vector3.zero,
				false, null, 0, null);
		}
		if (sound != null) {
			Play(clipName, delay);
	...
	/// 长时间没用的音效资源卸载 30秒
	/// </summary>
	static void CheckUnload() {
		RemoveAllSounds(_AllSounds, sound => !sound.currentSource.isPlaying && Time.time >= sound.lastPlayTime + 60);
	}
